...
export default function KnowledgeBasePanel() {
  ...
  // Only admins + powerusers manage knowledge; regular users get a read-only,
  // ACL-filtered view (the server only returns their permitted categories).
  const canManage = useStore(
    (s) => s.authUser?.role === "admin" || s.authUser?.role === "poweruser"
  );
      ...
      {isAdmin && (
        <div className="mb-4 rounded-xl border border-border-light p-3 dark:border-border-dark">
          ...
          <p className="mb-2 text-xs text-neutral-500">
            ...
            Namen korrigiert — z. B. „Msuter GmbH“ → „Muster GmbH“.
          </p>
          <textarea
            value={properNouns}
      ...
      {cats.length === 0 ? (
        <p className="py-4 text-center text-sm text-neutral-400">
          {canManage
            ...
            : "Dir wurden noch keine Wissenskategorien freigeschaltet."}
        </p>
      ) : (
