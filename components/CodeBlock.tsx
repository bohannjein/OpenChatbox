...
import { isSvgCode } from "@/lib/preview";
...
export default function CodeBlock({
  ...
  // A complete SVG renders as a live, interactive image (not raw XML). Only in
  // the chat (panel context present); the splitscreen's Code tab shows the source.
  // Wait until the closing </svg> has streamed so we never render a partial tree.
  if (panel && isSvgCode(code, language) && /<\/svg>\s*$/i.test(code)) {
    return <SvgView code={code} />;
  }
  ...
  return (
    <div className="my-4 rounded-lg border border-border-light dark:border-border-dark">
      ...
      {/* font-mono + whitespace-pre: ASCII art / console diagrams keep their exact
          columns and never reflow; the block scrolls horizontally instead. */}
      <pre className="overflow-x-auto whitespace-pre rounded-b-lg bg-[#f6f8fa] p-4 font-mono text-sm leading-6 dark:bg-[#0d1117]">
        {children}
      </pre>
    </div>
  );
