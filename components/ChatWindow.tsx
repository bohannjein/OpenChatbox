...
import { BASE_SYSTEM } from "@/lib/systemPrompt";
import {
  parseDocBlocks,
  stripDocBlocks,
  extractHtmlDoc,
...
export default function ChatWindow() {
  ...
  const buildSystem = (chatId: string, overrideSidekickId?: string) => {
    ...
    // Global natural-tone guidance leads every system prompt.
  ...
  const generate = async (chatId: string, assistantId: string) => {
    ...
    // Unified knowledge retrieval: the backend runs the local vector store (RAG,
    // restricted to this user's permitted categories) AND the linked BookStack
    // wiki IN PARALLEL, then merges + dedupes both into one consolidated context
    // with clear per-item source labels. The category ACL is enforced server-side
    // before any text is returned.
    let kbContext = "";
    if (kbEnabled && !sk && lastUser?.content?.trim()) {
      try {
        setMessagePipeline(chatId, assistantId, "knowledge");
        const r = await fetch("/api/kb/unified", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        if (typeof d.context === "string" && d.context) kbContext = d.context;
    ...
    // Combined external context (unified knowledge, web, then miss).
    const extraContext = [kbContext, searchContext, kbMiss]
      .filter(Boolean)
      .join("\n\n");
