import { NextRequest, NextResponse } from "next/server";
import { getUser } from "@/lib/server/adminAuth";
import { allowedCategoryIds } from "@/lib/server/users";
import { searchChunks, allCategoryIds } from "@/lib/server/kb";
import { embedOne } from "@/lib/server/embed";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/** Retrieve the most relevant KB chunks for a query, restricted to the caller's
 *  permitted categories (ACL). */
export async function POST(req: NextRequest) {
  const user = getUser(req);
  if (!user) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  const b = (await req.json().catch(() => ({}))) as { query?: string; k?: number };
  if (typeof b.query !== "string" || !b.query.trim())
    return NextResponse.json({ results: [] });
  const allowed = allowedCategoryIds(user, allCategoryIds());
  try {
    const emb = await embedOne(b.query);
    const results = searchChunks(emb, Math.min(Math.max(b.k ?? 8, 1), 12), allowed);
    return NextResponse.json({ results });
  } catch {
    // Embedding backend unavailable → no context (chat still answers).
    return NextResponse.json({ results: [] });
  }
}
