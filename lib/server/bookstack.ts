...
3. Proaktive Link-Verfolgung (Crucial Rule):
   - Wenn im Troubleshooting-Abschnitt der aktuellen Seite auf eine andere Wiki-Seite oder ein anderes Buch verwiesen wird (z. B. "Nutzen Sie stattdessen die Anleitung für Gerät B [Link/ID 456]"), darfst du nicht stoppen.
...
4. Lösungsorientierte Antwortstruktur: Formuliere deine Antwort bei Problemen immer proaktiv und biete Auswege an:
   - "Ich sehe, dass die Einrichtung von Gerät A bei dir fehlschlägt. Laut Wiki gibt es dafür folgenden Fallback: [Schritt-für-Schritt-Alternative aus dem Wiki]. Alternativ verweist der Eintrag auf die Anleitung für Gerät B. Soll ich diese für dich öffnen?"

RECHTSCHREIBUNG & AUTOMATISCHE KORREKTUR:
- Wenn ein Suchergebnis einen Hinweis auf eine automatische Korrektur enthält (der ursprüngliche Begriff ergab 0 Treffer und es wurde stattdessen ein korrigierter Begriff gesucht), weise den Nutzer charmant und knapp darauf hin, z. B.: "Ich habe in deinem Suchbegriff einen Tippfehler vermutet und stattdessen erfolgreich nach 'laufwerk backup' gesucht. Folgendes habe ich gefunden …". Verschweige die Korrektur nicht.`;
...
Beispiel-Input: "alufwerk backup"
Beispiel-Output: "laufwerk backup"`;
...
async function searchWithFallback(
  ...
  // admin dictionary). Runs FIRST: a mistyped proper noun ("Msuter GmbH") makes the
 ...
 */
/** A BookStack wiki page resolved to readable text — the structured unit the
 *  unified search merges alongside local RAG chunks. */
...
/**
 * Structured BookStack retrieval: search the wiki (with the fuzzy/spellcheck/
 * proper-noun fallback), read the top matching pages' Markdown, and return them
 * as structured pages (title/url/body). Empty when not configured or nothing is
 * found. `correctedQuery` is set when a typo/proper-noun correction was applied.
 */
export async function retrievePages(
  ...
  for (const it of targets) {
    ...
    if (!body) continue;
    out.push({ id, title: nm, url, body: body.slice(0, 2500) });
  }
  console.log(
  ...
  return { pages: out, correctedQuery };
}

/**
 * Formatted BookStack context block for the (single-source) KB-toggle route.
 * Thin wrapper over retrievePages(). The unified search formats pages itself.
 */
export async function retrieveContext(
  ...
  const blocks = pages.map(
    (p) => `(Quelle: ${p.title} — BookStack-ID ${p.id ?? "?"})\n${p.body}`
  );
  const sources: SourceLink[] = pages
    .filter((p) => p.url)
